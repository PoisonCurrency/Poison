...
{
    // Convert Poison:// to Poison:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Poison://"))
    {
        uri.replace(0, 12, "Poison:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Poison.lnk";
}
...
{
    return GetAutostartDir() / "Poison.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Poison-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Poison-qt [" + tr("command-line options") + "]                     " + "\n";
