...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2)
    {
        ...
        BOOST_FOREACH(Value& input, inputs)
        {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Poison address: ")+input.get_str());
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        if (pk.IsPayToScriptHash())
        {
            ...
            if (ExtractDestination(pk, address))
            {
				...
				//const CScriptID& hash = boost::get<CScriptID>(address);  // fix boost 1.58
